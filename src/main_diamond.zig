...
fn on_init() !void {
    ...
    const white_array = [1]u32{ delve.colors.white.toInt() };

    const tex_base = graphics.Texture.initFromBytes(1, 1, &white_array);

    // Load the emissive texture for the mesh
    const tex_emissive = graphics.Texture.initFromBytes(1, 1, &white_array);
    ...
    // Load our mesh!
    mesh_test = mesh.Mesh.initFromFile(delve.mem.getAllocator(), "assets/meshes/diamond.glb", .{ .material = material });
}
...
fn on_draw() void {
    ...
    model = model.mul(Mat4.rotate(time * 0.6, Vec3.new(0.5, 1.0, 0.3)));
    ...
    model = model.mul(Mat4.scale(Vec3.new(0.3, 0.3, 0.3)));
    mesh_test.?.draw(view_mats, model);

    model = Mat4.translate(Vec3.new(-2.0, 0.0, 0.0));
    model = model.mul(Mat4.rotate(time * 0.6, Vec3.new(0.3, 0.2, 1.0)));
    model = model.mul(Mat4.scale(Vec3.new(0.3, 0.3, 0.3)));
    mesh_test.?.draw(view_mats, model);
}
